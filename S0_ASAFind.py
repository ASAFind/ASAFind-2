...
# this file will be automaticall in tem directory as a part of final output file
...
####### targetp file will be ALWAYS provided by user ########
# !{targetp_dir}targetp -fasta {where_are_data}temp/{organism}_fasta_for_targetp2.fsa -org non-pl -format short -stdout > {where_are_data}signalp5_{organism}/fasta_{organism}.fsa.targetp2  

'''
# TargetP file must be added
if args.signalp_file == None:
    # Command to execute
    command = [f'{targetp_path}',
            '-fasta',
            f'{fasta_all_path}',
            '-org',
            'non-pl',
    ...
    with open(f'{temp_path}{fasta_file_name}.targetp2', 'w') as outfile:
        # Execute the command and redirect stdout to the file object        
        subprocess.run(command, stdout=outfile)
        print(command)
    signalp_file= f'{temp_path}{fasta_file_name}.targetp2'
    
    print("Command executed successfully.")
else:
    signalp_file = args.signalp_file 
'''  
signalp_file = args.signalp_file  
...
'''
f_cleared  = open(f'{fasta_file}.cleared.fsa', 'w')
for rec in SeqIO.parse(f"{fasta_file}", "fasta"): 
    if any(char in 'BZJUO-' for char in rec.seq) or len(rec.seq) < 25:
        continue 
    else:
        f_cleared.write(f'>{rec.id}\n{rec.seq}\n')  
f_cleared.close()

fasta_file = f'{fasta_file}.cleared.fsa'
'''
#####################Prepare scoring matrixes acc.to user input##########################################
'''
Prepare all variants of scoring matrixes acc.to user input

1.	No -PPC, no - fasta_file_with_motifs, no - fasta_file_with_motifs_ppc
Program uses default diatom scoring matrix for background only

2.	-PPC , no - fasta_file_with_motifs, no - fasta_file_with_motifs_ppc
Program uses both default diatom scoring matrix for background and ppc scoring matrix for background (images are doubled)

3.	No -PPC, - fasta_file_with_motifs, no - fasta_file_with_motifs_ppc
Program uses only fasta_file_with_motifs  and use S2 to gererate user_defined-diatom scoring matrix as background

4.-PPC, no - fasta_file_with_motifs, - fasta_file_with_motifs_ppc
Program uses default diatom scoring matrix and use S2 to gererate user_defined ppc scoring matrix as background (images are doubled)

5.-PPC, - fasta_file_with_motifs, - fasta_file_with_motifs_ppc
Program uses  fasta_file_with_motifs  and use S2 to gererate user_defined-diatom scoring matrix and uses  fasta_file_with_motifs_ppc  and use S2 to gererate user_defined ppc scoring matrix as background (images are doubled)
